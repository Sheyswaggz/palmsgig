...
export function useDisconnectSocialAccount() {
  ...
  return useMutation({
    mutationFn: async (platform: string) => {
      const response = await disconnectSocialAccount(platform);
      return { platform, success: response.data.success };
    },
    onSuccess: ({ platform }) => {
      // Remove the account from cache by platform
      queryClient.setQueryData<SocialAccount[]>(
        profileKeys.socialAccounts(),
        (oldData) => {
          if (!oldData) return [];
          return oldData.filter((account) => account.platform !== platform);
        }
      );
...
export function useVerifySocialAccount() {
  ...
  return useMutation({
    mutationFn: async (platform: string) => {
      const response = await verifySocialAccount(platform);
      return response.data;
    },
      ...
      queryClient.setQueryData<SocialAccount[]>(
        ...
        (oldData) => {
          ...
          return oldData.map((account) =>
            account.platform === verifiedAccount.platform ? verifiedAccount : account
          );
        }
      );
...
export function useRefreshSocialAccount() {
  ...
  return useMutation({
    mutationFn: async (platform: string) => {
      const response = await refreshSocialAccount(platform);
      return response.data;
    },
    onSuccess: (refreshedAccount) => {
      ...
      queryClient.setQueryData<SocialAccount[]>(
        ...
        (oldData) => {
          ...
          return oldData.map((account) =>
            account.platform === refreshedAccount.platform ? refreshedAccount : account
          );
        }
      );
...
export function useProfile() {
  ...
  return {
    ...
    walletBalance: wallet.data?.balance ? Number(wallet.data.balance) : undefined,
