...
class SocialAccount(BaseModel):
    ...
    def _get_encryption_key() -> bytes:
        """
        Get encryption key from settings (loaded from .env).
        ...
        """
        logger.debug("Attempting to retrieve SOCIAL_MEDIA_ENCRYPTION_KEY from settings")
        settings = get_settings()
        key_str = settings.SOCIAL_MEDIA_ENCRYPTION_KEY
        if not key_str:
            logger.error(
                "Encryption key not configured in settings",
                extra={"setting_checked": "SOCIAL_MEDIA_ENCRYPTION_KEY", "key_found": False}
            )
            raise ValueError(
                "SOCIAL_MEDIA_ENCRYPTION_KEY must be set in .env "
                "for token encryption/decryption"
            )

        logger.debug(
            "Encryption key found",
            extra={"key_length": len(key_str), "key_preview": f"{key_str[:4]}...{key_str[-4:]}"}
        )

        try:
            decoded_key = base64.urlsafe_b64decode(key_str)
            logger.debug("Encryption key decoded successfully", extra={"decoded_length": len(decoded_key)})
            return decoded_key
        except Exception as exc:
            logger.error("Invalid encryption key format", extra={"error": str(exc), "key_length": len(key_str)})
    ...
    def encrypt_token(token: str) -> str:
        ...
        Example:
            ...
            True
        """
        logger.info("Starting token encryption", extra={"token_length": len(token) if token else 0})
        try:
            logger.debug("Getting encryption key for encrypt_token")
            key = SocialAccount._get_encryption_key()
            logger.debug("Creating Fernet instance")
            fernet = Fernet(key)
            logger.debug("Encrypting token bytes")
            encrypted_bytes = fernet.encrypt(token.encode())
            encrypted_str = base64.urlsafe_b64encode(encrypted_bytes).decode()

            logger.info("Token encrypted successfully", extra={"encrypted_length": len(encrypted_str)})
            return encrypted_str
        except ValueError:
            # Re-raise ValueError from _get_encryption_key without wrapping
            raise
        except Exception as exc:
            logger.error("Token encryption failed", extra={"error": str(exc), "error_type": type(exc).__name__})
    ...
    def decrypt_token(encrypted_token: str) -> str:
        ...
        Example:
            ...
            'my_token'
        """
        logger.info("Starting token decryption", extra={"encrypted_length": len(encrypted_token) if encrypted_token else 0})
        try:
            logger.debug("Getting encryption key for decrypt_token")
            key = SocialAccount._get_encryption_key()
            logger.debug("Creating Fernet instance")
            fernet = Fernet(key)
            logger.debug("Decoding and decrypting token")
            ...
            logger.error("Token decryption failed", extra={"error": str(exc), "error_type": type(exc).__name__})
