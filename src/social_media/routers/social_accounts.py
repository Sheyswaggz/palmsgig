...
@router.post(
    ...
    summary="Manually link a social media account",
    description="Connect a social media account with username only (unverified)",
)
async def link_manual_account(
    ...
    """
    Manually link a social media account with username.

    Creates an unverified account record. The account will remain unverified
    until the user completes the OAuth verification flow.

    Args:
        request: Manual link request with platform and username
        user_id: Authenticated user ID from dependency
        session: Database session

    Returns:
        Created account information (unverified)

    Raises:
        HTTPException 400: If platform is invalid or account already exists
    ...
    try:
        ...
        account = await account_service.link_manual_account(
            user_id=user_id,
            platform=request.platform,
            username=request.username,
        )
        ...
        logger.warning(
            "Manual account link validation failed",
            extra={
                "user_id": user_id,
                "platform": request.platform,
                "error": str(e),
...
@router.post(
    ...
    description="Fetch fresh profile data from the social media platform",
)
async def refresh_account(
    ...
    """
    Refresh social media account data from platform.

    Fetches the latest profile information (username, display name, etc.)
    from the social media platform and updates the local record.
