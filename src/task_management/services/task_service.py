...
class TaskService:
    ...
    async def create_task(self, creator_id: str, task_data: TaskCreate) -> Task:
        ...
        task = Task(
            ...
            platform=task_data.platform.value,  # Use enum value (lowercase string)
            task_type=task_data.task_type.value,  # Use enum value (lowercase string)
            budget=task_data.budget,
            service_fee=service_fee,
            total_cost=total_cost,
            max_performers=task_data.max_performers,
            current_performers=0,
            status=TaskStatusEnum.DRAFT.value,  # Use enum value (lowercase string)
    ...
    async def publish_task(
        ...
        previous_status = task.status
        task.status = TaskStatusEnum.PENDING_PAYMENT.value  # Use enum value (lowercase string)
