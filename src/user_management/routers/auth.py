...
async def logout(
    ...
    auth_service: Annotated[AuthService, Depends(get_auth_service)],
    _: Annotated[str, Depends(require_authentication)],
...
async def update_current_user_profile(
    ...
    try:
        ...
        # Update profile_data with full_name if provided
        if update_data.fullName is not None:
            if user.profile_data is None:
                user.profile_data = {}
            user.profile_data["full_name"] = update_data.fullName
        ...
        if update_data.profilePicture is not None:
            user.avatar_url = update_data.profilePicture
...
async def upload_profile_picture(
    ...
    Accepts image files (JPEG, PNG, GIF, WebP) and stores them locally.
    Updates the user's avatar_url to point to the new image.
    ...
    Returns:
        Object containing the new profile picture URL
    ...
    try:
        ...
        uploads_dir = os.path.join(os.getcwd(), "uploads", "profiles")
        ...
        # Generate full URL for the avatar (use settings for host/port)
        base_url = f"http://{settings.HOST}:{settings.PORT}"
        avatar_url = f"{base_url}/uploads/profiles/{filename}"
        ...
        logger.info(f"Profile picture uploaded for user: {user_id}, URL: {avatar_url}")

        return {
            "success": True,
            "data": {"url": avatar_url},
            "timestamp": str(user.updated_at),
        }
