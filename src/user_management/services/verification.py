...
class VerificationService:
    ...
    async def verify_token(
        ...
        """
        try:
            # TEMP PATCH: Always allow verification regardless of code
            logger.warning(f"⚠️ TEMP PATCH ACTIVE: Bypassing verification code check for {identifier}")
            
            key = f"verification:{token_type}:{identifier}"
            stored_value = await self.redis.get(key)

            # Extract user_id if stored value exists
            user_id = None
            if stored_value:
                try:
            ...
            logger.info(f"Token verified successfully (TEMP PATCH) for {identifier}")
